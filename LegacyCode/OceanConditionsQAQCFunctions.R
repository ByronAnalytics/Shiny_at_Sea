...
individual.plots <- function(ctd, to.plot, cols) {
  ...
  if(any(!is.na(ctd$flag[ctd$variable == to.plot[1]]))) {
       points(ctd$value[ctd$variable == to.plot[1] & !is.na(ctd$flag)], 
             ...
             col = ctd$color[ctd$variable == to.plot[1] & !is.na(ctd$flag)], 
             cex = 2, pch = 21, lwd = 3)
  }
  par(new=T)
  ...
  if(any(!is.na(ctd$flag[ctd$variable == to.plot[2]]))) {
      points(ctd$value[ctd$variable == to.plot[2] & !is.na(ctd$flag)], 
            ...
            col = ctd$color[ctd$variable == to.plot[2] & !is.na(ctd$flag)], 
            cex = 2, pch = 21, lwd = 3)
    }
...
format.ctd <- function(ctd) {
  # Takes the raw format of ctd data from collection and converts column names and formats 
  #  to be consistent with the data base format
  # GOING TO NEED TO DO SOMETHING ABOUT STATION
  colnames(ctd)[grep("emperature", colnames(ctd))] <- "temp"
  colnames(ctd)[grep("Salinity", colnames(ctd))] <- "salinity"
  colnames(ctd)[grep("Oxygen", colnames(ctd))] <- "dox"
  colnames(ctd)[grep("pH", colnames(ctd))] <- "ph"
  colnames(ctd)[grep("Chl.a..ug.L..WET", colnames(ctd))] <- "fluorometry"
  colnames(ctd)[grep("X..T", colnames(ctd))] <- "pct_light"
  colnames(ctd)[grep("Station", colnames(ctd))] <- "station"
  colnames(ctd)[grep("Date", colnames(ctd))] <- "sample_date"
  colnames(ctd)[grep("Depth", colnames(ctd))] <- "depth_meter"
  
  to.return <- c("station", "sample_date", "depth_meter", "temp", "salinity", "dox", "ph", 
                 "fluorometry", "pct_light")
...
run.raw.plots <- function(ctd, ctd.historical) {
  ...
  print("Quantiles for each variable are:") 
  print(flags)
  ...
  to.write <- flag.ctd[!is.na(flag.ctd$flag), c("station","sample_date", "variable", "depth_meter", "value", "low.10", "high.90", "flag", "color")]
  write.csv(to.write, paste("CTD_QAQC_FlaggedVals_", Sys.Date(), ".csv", sep=""), row.names = F)
